...
const Manager = () => {
    ...
    const savePassword = async () => {
        if (form.site.length > 3 && form.username.length > 3 && form.password.length > 3) {
            ...
            // If the password already exists, update it; if not, add it to the array
            setPasswordArray((prevPasswords) => {
                const exists = prevPasswords.some((item) => item.id === newPassword.id);
                if (exists) {
                    // Update existing password
                    return prevPasswords.map((item) => item.id === newPassword.id ? newPassword : item);
                } else {
                    // Add new password
   ...
   const editPassword = (id) => {
    ...
    if (passwordToEdit) {
        // Set the form with the entry's details to pre-populate for editing
